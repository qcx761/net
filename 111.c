...
void control_connection_thread(int control_sock){
    ...
    // 发送STOR命令（示例：上传文件）
    string filename="upload_test.txt"; // 替换为你要上传的文件名
    send_command(control_sock,("STOR "+filename+"\r\n").c_str());
    
    // 发送RETR命令（示例：下载文件）
    string retr_filename="test.txt"; // 替换为你要下载的文件名
...
void data_connection_thread(){
    ...
    // 处理STOR命令（上传文件）
    ifstream infile("upload_test.txt"); // 替换为你要上传的文件名
    if(infile.is_open()){
    ...
    // 处理RETR命令（下载文件）
    ofstream outfile("test.txt"); // 替换为你要下载的文件名
    if(outfile.is_open()){
        while((bytes_received=recv(data_sock,buffer,sizeof(buffer),0))>0){
